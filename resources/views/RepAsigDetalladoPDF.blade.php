...
<body>
    ...
    <div class="titulo">REPORTE DE DOCUMENTOS ASIGNADOS</div>
    ...
    @foreach($fechas as $f)
    <br>
    <p class="titulo_fecha"> {{$f->fecha_para}} - ( {{$f->cantidad}} Documentos Asignados) </p>  
    ...
    <table>
        ...
        <tbody>
               @php $nro=0; @endphp
                ...
                @php if($not->notificado!=null) {@endphp<td>Notificado {{$not->fecha_notificacion}}</td>@php }@endphp
                @php if($not->devuelto!=null) {@endphp<td>Devuelto</td>@php }@endphp
                @php if($not->devuelto==null && $not->notificado==null) {@endphp<td>-</td>@php }@endphp
